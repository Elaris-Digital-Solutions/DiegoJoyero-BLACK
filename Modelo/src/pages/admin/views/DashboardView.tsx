...
export function DashboardView() {
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    if (error) {
     ...
     const publicId = removedProduct
       ? removedProduct.imagePublicId ?? extractPublicIdFromUrl(removedProduct.imageUrl)
       : null;
     if (publicId) {
       try {
  ...
  const importProducts = async (entries: ProductInput[]) => {
    ...
    try {
      ...
      if (relevantInserted.length > 0) {
        setProducts((previous) => {
          const existingIds = new Set(relevantInserted.map((product: ProductRecord) => product.id));
