...
// Fixed canonical category list requested by the product owner. Always present and
// rendered in alphabetical order when shown in selects.
...
const SORTED_CATEGORY_OPTIONS = [...CATEGORY_OPTIONS].sort((a, b) => a.localeCompare(b, 'es'));

const buildCategoryOptions = () => SORTED_CATEGORY_OPTIONS;
...
export function ProductTable({
  ...
  // Ref for horizontal-scrolling table wrapper: when the pointer is over this element,
  // vertical wheel scrolling will be translated into horizontal scrolling.
  ...
  // When the pointer is over the table wrapper, translate vertical wheel -> horizontal scroll.
  useEffect(() => {
    ...
    const onWheel = (ev: WheelEvent) => {
      ...
      if (ev.shiftKey) return; // allow native behavior when holding Shift
      ev.preventDefault();
      const multiplier = 1;
      el.scrollLeft += ev.deltaY * multiplier;
    };
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      return [product.name, product.category, product.description].join(' ').toLowerCase().includes(term);
    });
  }, [products, searchTerm]);
  ...
  const handleSaveRow = async (id: string) => {
    ...
    if (stagedImage) {
      try {
        ...
        setSubmitError(error instanceof Error ? error.message : 'No se pudo subir la imagen seleccionada. Intenta nuevamente.');
        return;
      }
    }
    ...
    <Card className="min-w-0 border border-border/70 bg-card/80 shadow-sm">
      ...
      <CardContent className="space-y-5 min-w-0">
        ...
        <div className="hidden md:block">
          <div
            ref={tableWrapperRef}
            className="overflow-x-auto rounded-xl border border-border/70 bg-background"
          >
                    ...
                    <tr key={product.id} className="align-top hover:bg-muted/40">
                      ...
                      <td className="px-4 py-4 align-top">
                        <select
                          ...
                          className={cn(
                            ...
                            current.status === 'active' ? 'text-emerald-600' : 'text-muted-foreground'
                          )}
                        >
                      ...
                      <td className="px-4 py-4 align-top">
                        <div className="flex justify-end gap-2">
                          ...
                          >
                            {isSaving(product.id) ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
...
function AddProductModal({ open, onClose, onSubmit }: AddProductModalProps) {
  ...
  const [form, setForm] = useState<AddProductFormState>({
    ...
    price: '',
    stock: '',
    category: SORTED_CATEGORY_OPTIONS[0],
    status: 'active',
    imageUrl: '',
    imagePublicId: null,
    material: 'gold',
              ...
              <div className="space-y-2">
                {modalPreviewImage ? (
                  <div className="flex items-center gap-3">
                    ...
                    <Button type="button" size="sm" variant="ghost" onClick={() => handleChange('imageUrl', '')}>
                      Quitar imagen
                    </Button>
                  </div>
                ) : null}
                <div className="flex flex-wrap gap-2">
                  <Button type="button" size="sm" variant="outline" className="gap-2" onClick={handlePickImage}>
                    <UploadCloud className="h-3.5 w-3.5" />
                ...
                <p className="text-[0.7rem] lowercase tracking-normal text-muted-foreground">{imageHelpText}</p>
              </div>
            </label>
          </div>
